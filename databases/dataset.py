...
class DatasetGenerator():
	...
	def load_data(self, DIR):
		...
		wav_files = [x.split('/')[1] + '/' + x.split('/')[2] for x in wav_files]
	...
	def apply_train_val_split(self, val_size, random_state):
		...
		self.df_train, self.df_val = train_test_split(self.df_train, 
													  test_size=val_size, 
													  stratify=self.df.label_id.values,
													  random_state=random_state)
	...
	def process_wav_file(self, x, threshold_freq=5500, eps=1e-10):
		...
		freqs, times, spec = stft(wav, fs=L, nperseg = 256, noverlap = 0, nfft = 259, padded = True, boundary = None)
	...
	def build_dataset(self,mode):
		...
		for i in ids:
			...
			if mode != 'test':
				y_batch.append(df.label_id.values[i])
			print(ids[i],' -> ', (len(X_batch)*100/len(ids)))

		X_batch = np.array(X_batch)

		np.save(mode+'_batches.npy',X_batch)
		if mode != 'test': 
			y_batch = to_categorical(y_batch, num_classes = len(self.label_set))
			np.save(mode+'_targets.npy',y_batch)
